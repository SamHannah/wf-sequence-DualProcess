...
# and for some number of simulated subjects
# Uses a DUAL-PROCESS version of MAL, with recall = test of normed echo against memory using high, fixed criterion and recognition =
# test of unnormed resubmitted echo against low, variable criterion.
...
sequence<- function(SF1, dp, L, A, N_subjects){
  ...
  highCrit<- 0; lowCrit<- 0; crit1<- 0; crit2<- 0; crit3<- 0    
  hcStart<-0.325                            # mean for Gaussian controlling setting of initial high criterion position
  hcVar<-0.025                              # variance for above   
  lcStart<-0.5                              # mean for Gaussian controlling setting of initial low criterion position
  lcVar<-0.1                                # variance for above   
  ...
  # eEturns the Hintzman similarity value and the tested vector: for use in picking the most similar vector in memory to a probe
  getSam<-function(VectA, VectB) {
    VectC<-abs(VectA)+abs(VectB)
    nR<- length(VectC[VectC != 0])
    return(c((VectA %*% VectB)/nR, VectB))
  }  
  #constructs a single echo, normalizes it, computes discrepancy with studied item, and then
  # applies probabilistic weighting of elements for encoding. Is applied row-wise to memory matrix using apply(), and contents of 
  # output summed up into a single vector using t(rowSums`(Echo))
  constructE<- function(z){
    Act<- getSim(Probe,z)^3 
    Echo<- Echo+(Act*z)
    return(Echo)
  }
  ...
  for (N_reps in 1: N_subjects) {
    highCrit<- rnorm(1, hcStart, hcVar)
    lowCrit<- rnorm(1, lcStart, lcVar)
    crit1<- lowCrit                                                # crit1 used to record initial crit setting
    SF<- SF1
    ...
    for (j in 1:N_study) {
      ...
      EchoM<- apply(Memory, 1, constructE )
      Echo<- as.vector(t(rowSums(EchoM)))
    ...
    for (j in 1:N_test) {
      ...
      EchoM<- apply(Memory, 1, constructE )
      Echo<- as.vector(t(rowSums(EchoM)))
      Intensity<- getSim(Probe, Echo)   #strength intensity
     # recognition with normed Echo
       Echo<- Echo/max(abs(Echo)) 
       Intensity<- getSim(Probe, Echo)             # clarity intensity
       Evidence<- Intensity - lowCrit       
       lowCrit<- lowCrit + SF*tanh(Evidence)^3
       SF<- SF - dp*SF      
      ## recall: test normed, resubmitted echo with fixed, high criterion tested against individual traces  (or, resubmit probe....)    
     if (Evidence <= 0) { 
      # Probe<- Echo      
       Echo<- vector(, N_el)
       EchoM<- apply(Memory, 1, constructE )
       Echo<- as.vector(t(rowSums(EchoM)))
       Echo<- Echo/max(abs(Echo))         
       rTest<- apply(Memory, 1, function(x) getSam(Echo, x) )  # returns a matrix of 101 X M, where M = # of memory traces; first row element = similarity
       rSim<- max(rTest[1,])
       maxTrace<- rTest[2:101, which(rTest[1,]== rSim) ]
       if (rSim > highCrit){          
         if (getSim(recallProbe, maxTrace) > highCrit){
           Evidence<- 1.0                                     # recall to accept
         }else{
           Evidence<- -1.0                                     # recall to reject
         }
       }  
     }
      if ( (crit2 == 1 && crit1 <= lcStart-(lcStart*.2)) || (crit2 == 2 && crit1  >= lcStart+(lcStart*.2)) ){
        Critter[j,crit2]<- lowCrit    
        crit3<- 1        
      }       
  ...
  quartMeans<- apply(summaryQ, c(1,2), qMean)  # each row of quartMeans holds means for one test ntile across test items, with columns holding means for a test item across test quaters
