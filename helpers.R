  ...
  for (N_reps in 1: N_subjects) {
    ...
    for (j in 1:N_test) {
     ...
     # recognition with normed Echo
       ...
       SF<- SF - dp*SF      
      ## recall: test normed, resubmitted echo with fixed, high criterion tested against individual traces      
     if (Evidence <= 0) { 
       Probe<- Echo      
       Echo<- vector(, N_el)
       EchoM<- apply(Memory, 1, constructE )
       ...
       if (rSim > highCrit){          
         if (getSim(recallProbe, maxTrace) > lowCrit){
           Evidence<- 1.0                                     # recall to accept
         }else{
           Evidence<- -1.0                                     # recall to reject
         }
         #print(paste(N_reps,"  ", j, " ",rSim," ",Evidence))
       }  
     }
